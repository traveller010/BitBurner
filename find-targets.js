...
export function getPotentialTargets(ns, compareField = "revYield") {
    ...
    var hackableNodes = networkNodes.filter(node => {
        return canHack(ns, node) && canPenetrate(ns, node, cracks) && !node.includes("pserv") && !node.includes("hacknet")
    });
    ...
    // ====================================================
    // INTEGRATED DASHBOARD LOGGING ENGINE (Your Tracking Feature)
    // ====================================================
    let currentMaxMoneyWhale = "";
    let maxMoneyValue = 0;
    let currentLiquidWhale = "";
    let maxLiquidValue = 0;

    for (let node of nodesDesc) {
        if (node.maxMoney > maxMoneyValue) {
            maxMoneyValue = node.maxMoney;
            currentMaxMoneyWhale = node.node;
        }
        if (node.curMoney > maxLiquidValue) {
            maxLiquidValue = node.curMoney;
            currentLiquidWhale = node.node;
        }
    }

    // Bulk overwrite logs with readable numerical data formats
    if (currentMaxMoneyWhale) {
        ns.write("server-with-max-money.txt", `${currentMaxMoneyWhale}:${ns.format.number(maxMoneyValue)}`, "w");
    }
    if (currentLiquidWhale) {
        ns.write("server-with-most-money.txt", `${currentLiquidWhale}:${ns.format.number(maxLiquidValue)}`, "w");
    }
    // ====================================================
